...
- [[#spacemacs-configs][Spacemacs configs]]
  ...
  - [[#keyfrequency-counter][keyfrequency counter]]
  - [[#switch-off-line-numbers-globally][switch off line numbers globally]]
...
- [[#xah-parenthesis][Xah parenthesis]]
  ...
  - [[#xahs-cut-line-or-region][Xah's cut line or region]]
...
** switch off line numbers globally
If you want line numbers toggle locally, then use ~SPC t n n~ or use ~M-x linum-mode~

#+BEGIN_SRC emacs-lisp :results silent

  (setq-default dotspacemacs-line-numbers nil)
  (message "After: switch off line numbers globally")
...
#+begin_src emacs-lisp :results silent
  ...
  (global-set-key (kbd "<f7> x") 'xah-cut-line-or-region) ; cut

  (global-set-key (kbd "<f7> c") 'xah-copy-line-or-region) ; copy

  (global-set-key (kbd "<f7> v") 'yank) ; paste
#+end_src
